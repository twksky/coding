...
- (void)setupSubviews
{
    ...
    [_photoButton setImage:[UIImage imageNamed:@"照片"] forState:UIControlStateNormal];//TODO
//    [_photoButton setImage:[UIImage imageNamed:@"chatBar_more_photo2Selected"] forState:UIControlStateHighlighted];
    ...
    UIView *containerTakePhotoView = [[UIView alloc] initWithFrame:CGRectMake(insets * (buttonIndex + 1) + CHAT_BUTTON_SIZE * buttonIndex, 10, CHAT_BUTTON_SIZE, CHAT_BUTTON_SIZE + 30.0f)];
    _takePhotoButton =[UIButton buttonWithType:UIButtonTypeCustom];
    [_takePhotoButton setFrame:CGRectMake(0, 0, CHAT_BUTTON_SIZE , CHAT_BUTTON_SIZE)];
    [_takePhotoButton setImage:[UIImage imageNamed:@"拍照"] forState:UIControlStateNormal];//TODO
    //    [_photoButton setImage:[UIImage imageNamed:@"chatBar_more_photo2Selected"] forState:UIControlStateHighlighted];
    [_takePhotoButton addTarget:self action:@selector(takePhotoAction) forControlEvents:UIControlEventTouchUpInside];
    
    UILabel *takePhotoTitle = [[UILabel alloc] initWithFrame:CGRectMake(0, CHAT_BUTTON_SIZE, CHAT_BUTTON_SIZE, 30.0f)];
    takePhotoTitle.text = @"拍照";
    takePhotoTitle.textColor = [UIColor blackColor];
    takePhotoTitle.textAlignment = NSTextAlignmentCenter;
    takePhotoTitle.font = [UIFont systemFontOfSize:14.0f];
    
    [containerTakePhotoView addSubview:_takePhotoButton];
    [containerTakePhotoView addSubview:takePhotoTitle];
    [self addSubview:containerTakePhotoView];
    buttonIndex++;
...
//    [_takePicButton setImage:[UIImage imageNamed:@"chatBar_colorMore_camera"] forState:UIControlStateNormal];
    ...
    [_mTemplateButton setImage:[UIImage imageNamed:@"模版"] forState:UIControlStateNormal];//TODO
//    [_mTemplateButton setImage:[UIImage imageNamed:@"chatBar_mTemplateSelected"] forState:UIControlStateHighlighted];
    ...
    mTemplateTitle.text = @"模版";
    mTemplateTitle.textColor = [UIColor blackColor];
...
-(void)takePhotoAction{
    if (_delegate && [_delegate respondsToSelector:@selector(moreViewTakePhotoAction:)]) {
        [_delegate moreViewTakePhotoAction:self];
    }
}
