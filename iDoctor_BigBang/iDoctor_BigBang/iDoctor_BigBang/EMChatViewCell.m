...
- (void)setupSubviewsForMessageModel:(MessageModel *)messageModel
{
    ...
    if (messageModel.isSender) {
        ...
        [_retryButton setImage:[[UIImage imageWithName:@"发送失败"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal]  forState:UIControlStateNormal];
//        [_retryButton setImage:[UIImage imageWithName:@"发送失败"] forState:UIControlStateNormal];
    ...
    switch (messageModel.type) {
        case eMessageBodyType_Text:
        {
            if (messageModel.message.ext) {
                ...
                NSInteger type = [[messageModel.message.ext objectForKey:@"type"] integerValue];
                NSString *action = [messageModel.message.ext objectForKey:@"action"];
                
                if ([@"web_view_cmd" isEqualToString:action] && 999 == type) {
                    
                    return [[EMChatTemplateBubbleView alloc] init];
                }
                else {
                    
                    return [[EMChatExtTextBubbleView alloc] init];
                }
        ...
        case eMessageBodyType_Text:
        {
            if (messageModel.message.ext) {
                ...
                if ([@"web_view_cmd" isEqualToString:action] && 999 == type) {
                    ...
                    return [EMChatTemplateBubbleView heightForBubbleWithObject:messageModel];
                }
                else {
                    
                    return [EMChatExtTextBubbleView heightForBubbleWithObject:messageModel];
                }
