...
@property (nonatomic, strong) TemplateModel *editableTemplate;
...
- (instancetype)initWithTemplate:(TemplateModel *)templateModel {
    ...
    if (self) {
        ...
        self.editableTemplate = templateModel;
    }
...
- (void)viewDidLoad {
    ...
    [self setupWithTemplate:self.editableTemplate];
...
- (void)setupViews {
    ...
    [contentView addSubview:self.templateItemTitle];
    {
        ...
        [contentView addConstraint:[self.templateItemTitle autoPinEdgeToSuperviewEdge:ALEdgeRight withInset:10.0f]];
    }
...
- (void)setupWithTemplate:(TemplateModel *)templateModel {
...
//    [self setSelectedTemplateCategory:templateModel.category];
    ...
    self.diyQuestions = [templateModel.fields mutableCopy];
...
#pragma mark - AddTemplateDiyQuestionCellDelegate Methods
...
- (void)saveTemplate {
    ...
    self.editableTemplate.name = self.templateNameField.text;
    self.editableTemplate.age = YES;
    self.editableTemplate.sex = YES;
    self.editableTemplate.last_time = YES;
    self.editableTemplate.symptomDescription = YES;
    
    self.editableTemplate.images = self.updateButton.isSelected;
    self.editableTemplate.fields = self.diyQuestions;
//    self.editableTemplate.category = self.selectedTemplateCategory;//模板类别需求暂时取消
    
    [self showLoading];
    
    [[AccountManager sharedInstance] asyncUpdateTemplate:self.editableTemplate CompletionHandler:^(TemplateModel *templateModel) {
        [self dismissLoading];

        [self showHint:@"修改成功"];
        
        if (self.delegate && [self.delegate respondsToSelector:@selector(didUpdateTemplate:)]) {
            
            [self.delegate didUpdateTemplate:templateModel];
        }
...
- (UILabel *)templateCategoryTitle {
    ...
    if (!_templateCategoryTitle) {
        ...
        [_templateCategoryTitle setHidden:YES];// 模板库需求取消  暂时隐藏
    }
...
- (UIButton *)templateCategoryButton {
    ...
    if (!_templateCategoryButton) {
        ...
        [_templateCategoryButton setHidden:YES];// 模板库需求取消  暂时隐藏
    }
